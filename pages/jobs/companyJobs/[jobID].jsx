...
export default function CompanyJobs() {
    ...
    closeJob(jobID)
      .then((res) => {
        if (res.data.status === 200) {
          openNotification(notificationTypes.SUCCESS, "Job Closed");
          router.push("/jobs");
  ...
  const handleEdit = () => {
    ...
    updateJob(jobID, data)
      .then((res) => {
        if (res.data.status === 200) {
          openNotification(notificationTypes.SUCCESS, "Job Updated");
        } else if (res.data.status === 423) {
          openNotification(notificationTypes.ERROR, "Error", res.data.message);
        } else if (res.data.status === 424) {
          openNotification(notificationTypes.ERROR, "Error", res.data.message);
      ...
      <main class={`dashboard ${sidebarOpen ? "active" : ""}`}>
        ...
        {jobSection === 1 ? (
          job === null ? (
            ...
            <CollegesList collegeType={isOffered} jobID={jobID} />
          )
        ) : jobSection === 2 ? (
          job === null ? (
            <></>
            ...
            <OfferedCollegesList collegeType={isOffered} jobID={jobID} />
          )
        ) : job === null ? (
