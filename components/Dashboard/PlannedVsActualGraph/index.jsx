    ...
    getCompanyHires()
      .then((res) => {
        if (res.data.status === 200) {
          ...
          let plannedHires = responseData.map((item) =>
            item.noOfPlannedHires !== null ? item.noOfPlannedHires : 0
          );
          console.log(plannedHires);
          let actualHires = responseData.map((item) =>
            item.noOfHires !== null ? item.noOfHires : 0
          );
          setYears(years);
    ...
    <>
      {loading === true ? (
        ...
        <div className="mt-4 mb-5 bg-white rounded-lg shadow-lg p-4">
          ...
          <Bar
            data={
              plannedHires == null || actualHires == null ? dataempty : data
            }
            options={options}
          />
        </div>
